...
!define INSTALLDIRNAME "GitKeymaster"
...
!macro PreferExistingAppDir _dir
  ...
  ${OrIf} ${FileExists} "${_dir}\git-account-manager.exe"
    StrCpy $INSTDIR "${_dir}"
    Return
  ${EndIf}
!macroend
...
Function .onInit
 ...
 ; Set default install location (ASCII folder; display name stays PRODUCTNAME)
 !if "${INSTALLMODE}" == "perMachine"
 ${If} ${RunningX64}
 !if "${ARCH}" == "x64"
 ...
 ; 新装默认 GitKeymaster；若本机已有旧目录，覆盖旧位置，避免并排两份。
 ${If} $INSTDIR == "${PLACEHOLDER_INSTALL_DIR}"
 ${OrIf} $UpdateMode = 1
 ${OrIf} $INSTDIR == "$LOCALAPPDATA\${INSTALLDIRNAME}"
 ${OrIf} $INSTDIR == "$PROGRAMFILES64\${INSTALLDIRNAME}"
 ${OrIf} $INSTDIR == "$PROGRAMFILES\${INSTALLDIRNAME}"
